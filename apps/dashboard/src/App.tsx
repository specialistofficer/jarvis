...
function GrowthView({ growth, busy, onGenerate, onAssetAction }: {
  ...
  return <div className="simple-page growth-page">
    ...
    <section className="growth-status-hero panel">
      ...
      <div><span className="eyebrow">Approval successful</span><h2>{readyAssets.length} assets approved and ready</h2><p>Aapke existing drafts delete nahi hue. Approval ke baad woh <strong>Ready to publish</strong> queue mein move ho gaye.</p></div>
    ...
    {goal && <section className="growth-scoreboard">
      ...
      <div className="metric-card"><small>Ready to publish</small><strong>{readyAssets.length}</strong><p>{drafts.length} awaiting approval</p></div>
    ...
    <section className="panel publish-queue"><div className="section-title"><div><span className="eyebrow">Publish queue</span><h2>{readyAssets.length ? `${readyAssets.length} approved assets` : "No approved assets yet"}</h2><p>Yahi aapke woh 5 drafts hain. Approval ke baad inka status change hua hai.</p></div><span className="done-count">{readyAssets.length} ready</span></div><div className="asset-grid">{readyAssets.map((asset) => <article className="growth-asset ready_to_publish" key={asset.id}><div className="asset-head"><span>{asset.channel.replaceAll("_", " ")}</span><small>Approved ✓</small></div><h3>{asset.title}</h3><blockquote>{asset.hook}</blockquote><p>{asset.body}</p><div className="asset-cta"><small>Call to action</small><strong>{asset.cta}</strong></div><details><summary>Production notes and evidence</summary><p>{asset.production_notes}</p>{(JSON.parse(asset.source_urls_json || "[]") as string[]).map((url) => <a href={url} target="_blank" rel="noreferrer" key={url}>Evidence source ↗</a>)}</details><div className="asset-actions"><button onClick={() => void copyAsset(asset)}>{copied === asset.id ? "Copied ✓" : "Copy content"}</button><span className="waiting-label">Waiting for {asset.channel.replaceAll("_", " ")} connection</span></div></article>)}{!readyAssets.length && <div className="empty-state">Approve a draft and it will appear here.</div>}</div></section>
...
export function App() {
  ...
  const onGrowthAssetAction = async (asset: GrowthAsset, action: "approve" | "reject" | "mark_published", externalUrl?: string) => { if (!confirmBrowserAction(`${action.replaceAll("_", " ")} “${asset.title}”?`)) return; setBusy(true); setNotice(null); try { await api.growthAssetAction(asset.id, action, externalUrl); await refresh(); setNotice(action === "approve" ? `Approved: “${asset.title}” is now visible in Ready to publish.` : `Saved: “${asset.title}” was ${action.replaceAll("_", " ")}.`); } catch (cause) { setError(cause instanceof Error ? cause.message : "Asset action failed"); } finally { setBusy(false); } };
